...
function initialise(options) {
  ...
  if (dictionaryOptions && dictionaryOptions.file) {
    ...
    switch (dictionaryOptions.language) {
      case 'en-us':
        baseFile = path.join(require.resolve('dictionary-en'), '../index');
        break;
      case 'en-gb':
        // default - do nothing
        break;
      ...
      case 'es-es':
        ...
        break;
      case 'fr-fr':
        baseFile = path.join(require.resolve('dictionary-fr'), '../index');
        break;
      default:
...
async function checkWord(word, options) {
  ...
  if (word.indexOf('-')) {
    ...
    const promises = await Promise.all(
      subWords.map(subWord => spellchecker.spell(subWord))
    );
