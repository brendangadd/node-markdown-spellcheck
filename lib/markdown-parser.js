...
const trackingReplacer = require('./tracking-replacement');

module.exports = function markdownParser(src) {
  const textTokens = [];
  let currentIndex = 0;
  ...
  tracker.replaceAll(/<\/?[a-z0-9]+ ?([a-z]+="[^"]*" ?)*\/?>/i, ' ');
  tracker.replaceAll(/\{:([^}]+)\}/, ' '); // Ignore HTML element attribute specs
  src = tracker.replaceAll(
